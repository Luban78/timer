...
function renderMoveRows(displaySteps) {
  ...
  if (isDesktop) {
    /**
     * Desktop po zvětšení diagramu už nemá tolik vodorovného prostoru,
     * proto délku notace dělíme adaptivně do 2–4 řádků.
     * Kratší algoritmy zůstávají ve 2 řádcích, delší se rozloží přehledněji.
     */
    let rowCount = 2;

    if (displaySteps.length > 14) rowCount = 3;
    if (displaySteps.length > 21) rowCount = 4;

    movesPerRow = Math.max(1, Math.ceil(displaySteps.length / rowCount));
  } else {
    /**
     * Mobil necháváme bezpečně po šesti tazích na řádek.
     */
    movesPerRow = 6;
  }
